...
- **Tenancy is absolute.** Every domain table carries `tenant_id`; every domain query runs inside `TenantDb.tx()` (`src/core/tenancy/`), which binds the ambient tenant to the transaction via `set_config('app.tenant_id', $1, true)`. An agent turn runs under ONE transaction: `AgentService.handleMessage` wraps the whole generate loop in `TenantDb.turn()`, and every `tx()` inside it joins as a SAVEPOINT on that transaction. Fire-and-forget writes spawned inside a turn must use `TenantDb.detached()` (they must not race the turn's COMMIT). Postgres RLS (`tenant_isolation` policies, USING + WITH CHECK) is the fail-closed second layer: unbound queries return zero rows, unbound writes are rejected.
